...
class WebBaseLoader:
    ...
    def load(self):
        ...
        for url in self.web_paths:
            # 遍历抓取网页的内容
            scrped_data = self._scrape(url)
            metadata = {
                "source": url,
                "title": scrped_data["title"],
                "language": scrped_data["language"],
            }
            docs.append(
                Document(page_content=scrped_data["content"], metadata=metadata)
            )
        return docs
